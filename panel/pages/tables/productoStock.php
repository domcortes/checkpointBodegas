...
<?php
/** Si este HTML es igual en TOOOOODAS las plantillas o si se cambnian solo algunos parametros,
 * mejor pasarlo a un archivo para incluir, o una clase helper que te genere este html con los
 * parametros que necesitas, onda:
 *
 * $helper->getLayout('header', ['title'] => 'Lista de productos de Bodega '.$bodega )
 *
 * y el metodo en la clase helper que sea algo así:
 *
 * public getLayout( Array $params ) {
 *  return <<<EOF
 *      <!DOCTYPE html>
 *      <html>
 *          ...
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                    /**
                                     * Voy a hacer la solucion que sugeriste del contador a ver si era parecida...
                                    */

                                    $count = 0; // inicializar fuera del loop, o se sobreescribirá a cada pasada.
                                    while ($row = $sentencia2->fetch(PDO::FETCH_ASSOC))  {
                                        $count++; // sumas uno en cada vuelta
                                    ?>
                                        <tr>
                                            <!-- this is the way.. -->
                                            <form id="stock-<?php echo $count; /* se le concatena el numero para tener un ID unico en la plantilla..*/ ?>" action="">
                                                <td>
                                                    <p class="text-center"><?php echo $row['SKU']; ?></p>
                                                </td>
                                                <td><?php echo $row['DESCRIPCION'] ?></td>
                                                <td>
                                                    <textarea style="font-size: 12px;" class="form-control" name="descripcionProducto<?php echo $count;?>" id="descripcionProducto<?php echo $count;?>" cols="35" rows="3" placeholder="sin descripcion... x ahora... 😀"></textarea>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <!-- TODOS ESTOS IDS ESTAN "MALOS" PORQUE VAN A ESTAR REPETIDOS POR FILA! -->
                                                    <input type="hidden" value="<?php echo $row['Bodega'] ?>" id='<?php echo "bodega" . $count; ?>' name="bodega<?php echo $count;?>">
                                                    <!--Esta podria ser una solucion no se si "bodega" es unico -->
                                                    ...
                                                    <input type="hidden" value="<?php echo $row['preUd']; ?>" id="precioVenta<?php echo $count;?>" name="precioVenta<?php echo $count;?>">
                                                    <?php
                                                    /**
                                                     ...
                                                     * 1. eso del href="javascript:;" no se si se sigue usando...
                                                     * 2. MUCHOS PARAMETROS!! cuando pase esto opta por usar Arrays
                                                     * 3. Hice una funcion realizarProcesoFix() para que la uses como base.
                                                     */
                                                    ?>
                                                    <input type="button" onclick="realizaProceso(<?php echo $count; /* Asi sabemos que boton es este...*/ ?> )" value="Sumar a catalogo" class="btn btn-success btn-block">
                                                </td>
                                            </form>
                                        </tr>
                                    <?php
